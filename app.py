...
p_planned_transmission = "Planned_Project_Data.xlsx"
...
def get_transmission_data():
    xls_file = pd.ExcelFile(p_planned_transmission)
    df_planned = pd.read_excel(xls_file, 'NatGrid planned infra. layer')
    df_project = pd.read_excel(xls_file, 'Hitachi Projects Layer')
    df_proj1 = pd.read_excel(xls_file, 'HE-SS-1')
    df_proj2 = pd.read_excel(xls_file, 'HE-HV-1')
    return df_planned, df_project, df_proj1, df_proj2
...
<td style="width: 100px;background-color: """+ right_col_color +""";">{}</td>""".format(gnode_latitude) + """
</tr>
<tr>
<td style="background-color: """+ left_col_color +""";"><span style="color: #000000;">Longitude</span></td>
<td style="width: 100px;background-color: """+ right_col_color +""";">{}</td>""".format(gnode_longitude) + """
</tr>
</tbody>
</table>
</html>
...
def show_demand_map(base_map, year, demand_data, tile_select):
    map = folium.Map(location=[53.509865 , -0.218092], zoom_start=6, scrollWheelZoom=False, tiles='CartoDB positron')
    choropleth = folium.Choropleth(geo_data = json.load(open(p_overwrite_file)),
                                  data = demand_data,
...
def show_transmission_lines(mp,df_planned,df_project,df_proj1,df_proj2):   

    #pushpin = folium.features.CustomIcon('/content/drive/My Drive/Colab Notebooks/pushpin.png', icon_size=(30,30))

    for index,row in df_planned.iterrows():
        code = row['Code']
        proj_details = df_project.loc[df_project['HND Reference'] == code]
        if proj_details['Project ID'].any() == 'HE-SS-1':
            proj_comps = df_proj1
        else:
            proj_comps = df_proj2
        # sidebar_popup = sidebar_html(row,proj_details,proj_comps)
        loc_list = eval(row['Path'])
        folium.PolyLine(locations = loc_list,line_opacity = 0.5,color = row['Color']).add_to(mp)
        middleIndex = (len(loc_list))//2
        mp1 = folium.Marker(location=loc_list[middleIndex],icon =folium.Icon(color='black',icon='solid circle-info',prefix='fa'),)
        mp.add_child(mp1)
    fg_list = []
    i = 0
    capacity_list = df_planned.Capacity.unique()
    ...
    if st_map["last_object_clicked"]:
        regionlat = st_map["last_object_clicked"]["lat"]
        regionlng = st_map["last_object_clicked"]["lng"]
    ...
    return regionlat,regionlng, mp


def show_sidebar(lat,lon,planneddata,projectdata) :    

    #projectdata.rename(columns = {'HND Reference ':'Code'}, inplace = True

    for index,row in planneddata.iterrows():
        
        loc_list = eval(row['Path'])

        middleIndex = (len(loc_list))// 2

        location = loc_list[middleIndex]

        if location[0] == lat and location[1] == lon :

            code = row['Code']

            project = projectdata.loc[projectdata['HND Reference'] == code]
            ...
            with st.sidebar :
                ...
                st.write("**Description:**",row['Name'])  

                st.write("**Planned:**",row['Description'])  

                st.write("**Capacity:**",row['Capacity'])

                st.write("**Type:**",row['Type'])

                st.write("**Status:**",row['Status'])

                st.header("More Information Below :")

                st.write("**Project ID:**",project['Project ID'])  

                st.write("**Project Name:**",project['Project Name'])  

                st.write("**Project Type:**",project['ProjectType'])

                st.write("**Project Status:**",project['ProjectStatus'])

                st.write("**Project Manager:**",project['Project Manager'])
...
def app_layout():
    ...
    if select_data == "":
        ...
        demand_data = pd.read_csv(p_demand_file, sep=',')
        df_heatregion_merged_map_cpy = df_heatregion_merged_map.copy()
        df_heatregion_merged_map_cpy = pd.melt(df_heatregion_merged_map_cpy, id_vars=['GSP','geometry'])
        df_heatregion_merged_map_cpy = df_heatregion_merged_map_cpy.set_index('variable')
        yearlist = list(set(df_heatregion_merged_map_cpy.index))
        yearlist.sort()
        year = st.sidebar.selectbox('Choose the year', yearlist)
        region_name = show_demand_map(base_map, year, demand_data, tile_select)
        region_name
    elif select_data == "Transmission Network":
        df_planned, df_project, df_proj1, df_proj2 = get_transmission_data()
        lat,lon , map = show_transmission_lines(base_map,df_planned,df_project,df_proj1,df_proj2)
        ...
        if lat != '0' and lon != '0' :
            ...
            #st.write(lat,lon)
            show_sidebar(lat,lon,df_planned,df_project)
