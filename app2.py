...
p_gnode_name_asset = "gnode_name_assets.geojson"
...
def get_dataset():
    ...
    gnode_name_assets = p_gnode_name_asset
    gnode_assets = gpd.read_file(gnode_name_assets)
    gnode_assets = GeoDataFrame(gnode_assets, crs="EPSG:4326", geometry=gnode_assets['geometry'])
#     gnode_assets = gnode_assets[gnode_assets['geometry'] != None]
...
#         ),
#     ).add_to(mp)
    assetgeo = folium.GeoJson(
        heatmap,
        name="Gnode Point",
        style_function=style_function,
        tooltip=folium.GeoJsonTooltip(
            fields=["Gnode_name"], aliases=["Gnode name"], localize=True
        ),
    ).add_to(mp)
...
def app_layout():
    ...
    #fetch the data
    df_points, gnode_data_asset = get_dataset()
    #design for the app
    st.title('Select the Map to view')
    if select_data == "":
        map = base_map
    elif select_data == "Grid Supply Points":
        map = app_feature(base_map, tile_select, gnode_data_asset)
        folium_static(map)
